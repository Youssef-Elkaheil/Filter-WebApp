...
function setup() {
    ...
    maincircle = new Zero(width / 2, height / 2, 200);
    for (let i = -1; i < 2; i++) {
        allPass.push(new AllPass(200 + 50 * i, 150 + 50 * i * i))
        addFilter();
    }
}
function draw() {
    ...
    selectedFilters.splice(0,selectedFilters.lenght);
    for (let i = 0; i < allPass.length; i++) {
    ...
    fetch("/arrays", {
        ...
        body: JSON.stringify({ "z": zeroes, "p": poles, "all_pass": selectedFilters })
    }).then(res => {
